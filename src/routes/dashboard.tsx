...
const summary = [
  { key: "open",      label: "Open",      value: 24, delta: "+3", icon: Zap,           color: "text-primary",     bg: "bg-primary/10" },
  { key: "pending",   label: "Pending",   value: 12, delta: "−1", icon: Clock,         color: "text-warning",     bg: "bg-warning/15" },
  { key: "resolved",  label: "Resolved",  value: 86, delta: "+12", icon: CheckCircle2, color: "text-success",     bg: "bg-success/15" },
  { key: "escalated", label: "Escalated", value: 3,  delta: "+1",  icon: AlertTriangle, color: "text-destructive", bg: "bg-destructive/12" },
];
    ...
    <MobileShell>
      <div className="min-h-screen bg-background pb-32">
        ...
        <div className="relative px-5 pt-[calc(env(safe-area-inset-top,0px)+1.5rem)] pb-6 bg-gradient-hero text-white rounded-b-[2rem] overflow-hidden">
          ...
          <Link to="/tickets" className="relative mt-5 flex items-center gap-2 h-12 rounded-2xl glass-dark px-4 text-white/70 text-sm">
          ...
          <div className="relative mt-4 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/15 p-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-white/60 truncate">AT-2835 • Production DB latency — 32m remaining</p>
            </div>
        ...
        <div className="px-5 mt-5">
          <div className="grid grid-cols-2 gap-3">
            {summary.map((s) => {
              const Icon = s.icon;
              return (
                <Link to="/tickets" key={s.key} className="rounded-3xl bg-card border border-border p-4 shadow-soft hover:shadow-elevated transition-all">
                  ...
                  <p className="mt-3 text-[28px] font-extrabold tracking-tight leading-none">{s.value}</p>
        ...
        <div className="px-5 mt-6">
          <div className="grid grid-cols-4 gap-2.5">
            {[
              ...
              <Link key={a.label} to={a.to} className="flex flex-col items-center gap-1.5 py-3 rounded-2xl bg-card border border-border">
        ...
        <div className="px-5 mt-7 flex items-center justify-between">
          ...
          <Link to="/profile" className="text-[12px] text-primary font-semibold">View team</Link>
        </div>
        <div className="px-5 mt-3 flex gap-3 overflow-x-auto no-scrollbar pb-1">
          {technicians.map((t) => (
            <div key={t.name} className="shrink-0 w-[140px] rounded-2xl bg-card border border-border p-3">
              <div className="relative h-11 w-11 rounded-2xl bg-gradient-brand text-white flex items-center justify-center font-bold">
                {t.initials}
                <span className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full ring-2 ring-card ${t.online ? "bg-success" : "bg-muted-foreground"}`} />
              </div>
        ...
        <div className="px-5 mt-7 flex items-center justify-between">
          ...
          <Link to="/tickets" className="text-[12px] text-primary font-semibold">See all</Link>
        </div>
        <div className="px-5 mt-3 space-y-2.5">
          {tickets.slice(0, 4).map((t) => (
            <Link key={t.id} to="/tickets/$id" params={{ id: t.id }} className="block rounded-2xl bg-card border border-border p-4 active:scale-[0.99] transition-transform">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-muted-foreground tracking-wider">{t.id}</span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusStyles[t.status].bg} ${statusStyles[t.status].text}`}>{statusStyles[t.status].label}</span>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-muted-foreground">{priorityStyles[t.priority].label} · {t.category}</span>
                </div>
