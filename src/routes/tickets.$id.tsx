    ...
    <div className="h-full w-full max-w-7xl mx-auto bg-background p-4 xl:p-6 flex flex-col overflow-hidden">
      {ticket && (
        <div className="flex shrink-0 items-center gap-4 mb-6">
          <button
            onClick={() => navigate({ to: "/dashboard" })} // Type hack to avoid tight coupling
          ...
          <span
            className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${priorityBadge?.bg} ${priorityBadge?.text}`}
          >
            {ticket.priority}
          </span>
          <span
            className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${getStatusBadgeClass(ticket.status)}`}
          >
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-10 gap-6">
                    ...
                    <div className="bg-amber-500/10 text-amber-900 dark:text-amber-200 border border-amber-500/20 rounded-xl p-4 font-medium text-xs w-full sm:w-[85%] flex flex-col gap-2 shadow-sm">
                      <div className="flex items-center gap-2 opacity-80 mb-1">
                        ...
                        <span className="uppercase tracking-widest font-bold text-[10px]">
                          Staff-Only Internal Note • {msg.senderName}
                        </span>
                      </div>
          ...
          <div className="p-4 border-t border-border bg-card shrink-0">
            <Tabs defaultValue="public" className="w-full">
              <TabsList className="mb-3">
                ...
                <TabsTrigger
                  value="internal"
                  className="text-amber-600 data-[state=active]:bg-amber-500/10 data-[state=active]:text-amber-700"
                >
                  Internal Staff Note
                </TabsTrigger>
              </TabsList>
        ...
        <div className="xl:col-span-3 flex flex-col gap-5 bg-card p-5 rounded-xl border border-border shadow-xs overflow-y-auto h-full">
          {ticket?.slaDeadline && (
            <div
              className={`p-4 rounded-xl border flex flex-col gap-1 ${slaRisk ? "bg-destructive/10 border-destructive/20 text-destructive" : "bg-muted/40 border-border text-foreground"}`}
            >
              <div className="flex items-center gap-2 opacity-80 text-[11px] font-bold uppercase tracking-wider">
                <Clock className="w-3.5 h-3.5" />
                SLA Deadline Contract
              </div>
              <div className="text-xl font-bold tracking-tight">{slaText}</div>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value={ticket?.assignedToId || ""}>
                  {ticket?.assignedToName || "Alex Rivera (Tech)"}
                </option>
              </select>
            </div>
