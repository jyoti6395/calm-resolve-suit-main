...
import { Paperclip, Mic, Send, MoreVertical, AlertTriangle, RefreshCcw, Clock, CheckCheck, Image as ImageIcon } from "lucide-react";

export const Route = createFileRoute("/tickets/$id")({ component: TicketDetail });

const messages = [
  { from: "user", text: "Hey — VPN keeps dropping every ~10 minutes on my MacBook (M2, Sonoma 14.5). Reinstalled AnyConnect already.", time: "14:02" },
  { from: "tech", text: "Thanks for the details, Alex 👋  Looking at your client logs now. Can you grab a screenshot of the AnyConnect status window when it drops?", time: "14:04" },
  { from: "user", text: "Sure — uploading now.", time: "14:05" },
  { from: "user", attachment: "screenshot-14-05.png", time: "14:05" },
  { from: "tech", text: "Got it. Looks like the EAP-FAST handshake is timing out. I'm pushing a profile update — please reconnect once.", time: "14:09", read: true },
];
  ...
  return (
    <MobileShell>
      <div className="min-h-screen bg-background flex flex-col pb-24">
        <AppHeader title={t.id} subtitle={t.category} back right={
        ...
        <div className="mx-5 rounded-3xl bg-gradient-hero text-white p-5 shadow-elevated relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/15 backdrop-blur`}>{statusStyles[t.status].label}</span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/15 backdrop-blur">{priorityStyles[t.priority].label}</span>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="h-9 w-9 rounded-full bg-white/15 backdrop-blur flex items-center justify-center text-[12px] font-bold">{t.assignee.initials}</div>
                <div>
              ...
              <div className="text-right">
                ...
                <p className="text-[14px] font-bold flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {t.slaHours}h 12m</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-5 mt-5 p-1 rounded-2xl bg-secondary grid grid-cols-3">
          {(["chat", "timeline", "details"] as const).map((k) => (
            <button key={k} onClick={() => setTab(k)} className={`h-9 rounded-xl text-[12px] font-semibold capitalize transition-all ${tab === k ? "bg-card text-foreground shadow-soft" : "text-muted-foreground"}`}>
              {k}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-3">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 ${m.from === "user" ? "bg-primary text-primary-foreground rounded-br-md" : "bg-card border border-border rounded-bl-md"}`}>
                    {m.attachment ? (
                      <div className="rounded-xl bg-white/15 p-3 flex items-center gap-2">
                        <div className="h-9 w-9 rounded-lg bg-white/20 flex items-center justify-center"><ImageIcon className="h-4 w-4" /></div>
                        <div>
                    ...
                    <div className={`mt-1 flex items-center gap-1 text-[10px] ${m.from === "user" ? "text-white/70" : "text-muted-foreground"}`}>
                      {m.time}
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[440px] px-3 pb-3 pt-2 bg-gradient-to-t from-background via-background/90 to-transparent">
        <div className="glass rounded-2xl flex items-center gap-1 px-2 py-1.5 shadow-elevated">
          <button className="h-10 w-10 rounded-xl hover:bg-muted flex items-center justify-center"><Paperclip className="h-4 w-4" /></button>
          <input placeholder="Type a message…" className="flex-1 bg-transparent outline-none px-2 text-[14px]" />
          <button className="h-10 w-10 rounded-xl hover:bg-muted flex items-center justify-center"><Mic className="h-4 w-4" /></button>
          <button className="h-10 w-10 rounded-xl bg-gradient-brand text-primary-foreground flex items-center justify-center"><Send className="h-4 w-4" /></button>
        </div>
      </div>
    </MobileShell>
  );
